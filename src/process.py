...
# def add_players_info(match_pbp):
#     '''
#     Args:
#     -----
...
#     Collect all playersData (player_ids) information for a match into a pandas DataFrame

#     '''
#     match_data = match_pbp['courtVisionData'][0]['playersData']
    
    
#     players_data_df = pd.DataFrame()
#     players_data_df['name'] = match_data['playersData']['playerTeam']['name']
#     players_data_df['id'] = match_data['playersData']['playerTeam']['id']
#     players_data_df['country'] = match_data['playersData']['playerTeam']['country']
#     players_data_df['league']
#     players_data_df['player_handedness']
    
#     return players_data_df

def get_players_catalogue(match_pbp):
    '''
    Args:
    -----
    match_pbp [pandas DataFrame]
    ...
    Collect all playersData (player_ids) information for all match into a pandas DataFrame
    
    "name","id","country","league","player_handedness"
...
def get_match_info(raw_json_file, tournament, league, match_id, date):
    '''
    Args:
    -----
    one_point_sequence [dict]: Dictionary
    ...
    Collect match catalogue data information for a match into a pandas DataFrame

    year,player1,player2,player1_id,player1_country,player1_seed,player2_id,player2_country,player2_seed,court_name,court_id,num_sets_completed,match_type,match_status,match_id,tournament,league
    
    ,,,,,,,,,,,,,,,,league

    '''
    all_match_data = raw_json_file['courtVisionData'][0]
    
    match_data = {
        'year': [date],
        'player1': [all_match_data['playersData']['playerTeam'][0]['name']],
        'player2': [all_match_data['playersData']['opponentTeam'][0]['name']],
        'player1_id': [all_match_data['playersData']['playerTeam'][0]['id']],
        'player2_id': [all_match_data['playersData']['opponentTeam'][0]['id']],
        'player1_country': [all_match_data['playersData']['playerTeam'][0]['country']],
        'player2_country': [all_match_data['playersData']['opponentTeam'][0]['country']],
        'player1_seed': [all_match_data['playersData']['playerTeam'][0]['seed']],
        'player2_seed': [all_match_data['playersData']['opponentTeam'][0]['seed']],
        'court_name': [all_match_data['courtName']],
        'court_id': [all_match_data['courtId']],
        'num_sets_completed': [all_match_data['setsCompleted']],
        'match_type': [all_match_data['eventType']],
        'match_status': [all_match_data['matchStatus']],
        'match_id': [match_id],
    ...
    match_catalogue_data = pd.DataFrame(match_data)
    
    return match_catalogue_data
...
def get_ranking_info(raw_json_file):
    ...
    Returns:
    --------
    pandas DataFrame with cumulative ranking history
    ...
    ranking_history = [
        {
            **record,
            "RankDate": pd.to_datetime(record["RankDate"]).strftime("%d-%m-%Y"),
            "FirstRankYear": raw_json_file['FirstRankYear'],
            "LastRankYear": raw_json_file['LastRankYear']
        }
        for record in raw_json_file['History']
    ]

    df = pd.DataFrame(ranking_history)

    column_mapping = {
        "RankDate": "rank_date",
        "SglRollRank": "singles_roll_rank",
        "SglRollTie": "singles_roll_tie",
        "SglRollPoints": "singles_roll_points",
        "SglRaceRank": "singles_race_rank",
        "SglRaceTie": "singles_race_tie",
        "SglRacePoints": "singles_race_points",
        "DblRollRank": "doubles_roll_rank",
        "DblRollTie": "doubles_roll_tie",
        "DblRollPoints": "doubles_roll_points",
        "FirstRankYear": "first_rank_year",
        "LastRankYear": "last_rank_year"
    }
...
def add_player_name_by_id(df_pbp):
    ...
    df_pbp["player1_name"] = df_pbp["player1"].map(player_id_name).fillna(df_pbp["server_id"])
    df_pbp["player2_name"] = df_pbp["player2"].map(player_id_name).fillna(df_pbp["returner_id"])
